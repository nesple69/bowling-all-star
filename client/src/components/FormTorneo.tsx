...
const FormTorneo: React.FC = () => {
            ...
            try {
                ...
                if (isEdit) {
                    ...
                    setFormData({
                        ...
                        dataFine: t.dataFine ? format(new Date(t.dataFine), 'yyyy-MM-dd') : '',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setStatus({ type: 'error', message: err.response?.data?.message || 'Errore durante il salvataggio.' });
        } finally {
            setIsLoading(false);
        }
    };
                                                ...
                                                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <input
                                                            ...
                                                            required
                                                            className="w-full px-4 py-2 bg-white border border-gray-100 rounded-xl font-bold text-sm outline-none focus:ring-2 focus:ring-primary/20"
                                                            value={format(new Date(editingTurno.orarioInizio), 'HH:mm')}
                                                            onChange={e => {
                                                                const time = e.target.value;
                                                                const fullDate = `${format(new Date(editingTurno.giorno), 'yyyy-MM-dd')}T${time}:00`;
                                                    ...
                                                    <div>
                                                        ...
                                                        <input
                                                            ...
                                                            required
                                                            className="w-full px-4 py-2 bg-white border border-gray-100 rounded-xl font-bold text-sm outline-none focus:ring-2 focus:ring-primary/20"
                                                            value={format(new Date(editingTurno.orarioFine), 'HH:mm')}
                                                            onChange={e => {
                                                                const time = e.target.value;
                                                                const fullDate = `${format(new Date(editingTurno.giorno), 'yyyy-MM-dd')}T${time}:00`;
                                        ...
                                        <div key={turno.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 group transition-all hover:bg-white hover:shadow-md hover:border-gray-200">
                                            <div className="flex items-center gap-6">
                                                <div className="flex flex-col">
                                                    ...
                                                    <span className="font-bold text-sm">{format(new Date(turno.giorno), 'dd MMM yyyy', { locale: it })}</span>
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-0.5">Orario</span>
                                                    <span className="font-bold text-sm">
                                                        {format(new Date(turno.orarioInizio), 'HH:mm')} - {format(new Date(turno.orarioFine), 'HH:mm')}
                                                    </span>
                                                </div>
                                                <div className="flex flex-col">
